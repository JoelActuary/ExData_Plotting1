...
## Data frame with just date_time and Global active power
plot_data <- data.frame(strptime(data_df$Date_Time,"%d/%m/%Y %H:%M:%S"),data_df$Global_active_power)
colnames(plot_data) <- c("Date_Time", "Global Active Power (Kilowatts)")

## Plot line graph of date_time (x) vs Global Active Power (y)
...
par(mar=c(5,5,4,3))
plot(x = plot_data$Date_Time, y = plot_data$`Global Active Power`, 
     type = "l", 
     xlab = "", 
